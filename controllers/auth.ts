...
export const regAuth = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { first_name, last_name, password, email }: userObj = req.body;
    ...
    if (password) {
      ...
      const user: userOmit = await knex
        .insert({
...
export const logAuth = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    res.status(200).json({
      success: true,
      msg: 'logAuth',
